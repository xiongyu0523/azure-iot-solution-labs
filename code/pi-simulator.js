...
function getMessage(cb) {
  ...
  sensor.readSensorData()
    .then(function (data) {
      const temp = Math.floor(data.temperature_C * 100).toString(16).padStart(4, '0');
      const humi = Math.floor(data.humidity * 100).toString(16).padStart(4, '0');
      cb(JSON.stringify({ type: 'cycCan', payload: { c1: `010304${temp}${humi}00`} }));
    })
    .catch(function (err) {
